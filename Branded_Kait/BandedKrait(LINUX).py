...
def write_to_target(stanza, backup_dir, archive_dir):
    """Копирование данных в целевую директорию с обновлением статуса для Zabbix"""
    try:
        ...
        # Копируем backup
        rsync_backup = f"rsync -av --progress {backup_dir}/ {backup_target}/"
        logging.info(f"Копирование из {backup_dir} в {backup_target}")
        update_zabbix_status(backup_dir, 0, "в процессе")
        backup_result = subprocess.run(rsync_backup, shell=True, check=True)
        update_zabbix_status(backup_dir, 50, "в процессе")

        # Копируем archive
        rsync_archive = f"rsync -av --progress {archive_dir}/ {archive_target}/"
        logging.info(f"Копирование из {archive_dir} в {archive_target}")
        archive_result = subprocess.run(rsync_archive, shell=True, check=True)
        ...
        if backup_result.returncode == 0 and archive_result.returncode == 0:
            ...
            logging.error(f"Ошибка записи для {stanza}: rsync завершился с ошибкой.")
            return False
    except subprocess.CalledProcessError as e:
        update_zabbix_status(stanza, 0, "ошибка")
        logging.error(f"Ошибка при выполнении rsync для {stanza}: {e}")
        return False
